...
def find_best_chunk(question, model_answer, chunks, embed_model, topic_filer=None):
    if topic_filer:
        filterend_chunks = [c for c in chunks if topic_filer.lower() in c.lower()]
    ...
    answer_emb = embed_model.encode([model_answer])[0]
